...
function _init() {
  showNbFeatures =
    configuration.getConfiguration().application.showClickNbItems !== "false";
...
function refreshResultsNumber() {
  if (
    showNbFeatures &&
    info.getQueriedFeatures() != undefined &&
    info.getQueriedFeatures().length > 1
  ) {
    $("#popup-number-results").html(
      info.getQueriedFeatures().length + " résultats",
    );
