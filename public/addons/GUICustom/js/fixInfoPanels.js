...
var observer = new MutationObserver(function (mutations) {
  if (mutations.length >= 2) {
    if (mutations[0].target.id != mutations[1].target.id) {
      setTimeout(function () {
        if (
          mutations[0].target.classList[0] === "active" &&
          mutations[1].target.classList[0] === "active"
        ) {
          var panelWidth;
          var panelHeight;
          mutations.forEach((mutation, i) => {
            if (mutation.target.id == "right-panel") {
              panelWidth = document.getElementById(
                mutation.target.id,
              ).offsetWidth;
            }
            if (mutation.target.id == "bottom-panel") {
              panelHeight = document.getElementById(
                mutation.target.id,
              ).offsetHeight;
            }
          ...
          decaleMap([
            parseInt(panelWidth, 10) / 3,
            (parseInt(panelHeight, 10) / 3) * -1,
          ]);
        }
        if (
          mutations[0].target.classList[0] != "active" &&
          ...
          if (
            mutations[0].target.id == "right-panel" &&
            mutations[1].target.id == "bottom-panel"
          ) {
            //On ferme right on ouvre bottom
            var panelHeight = document.getElementById(
              mutations[1].target.id,
            ).offsetHeight;
            var panelWidth = document.getElementById(
              mutations[0].target.id,
            ).offsetWidth;
          ...
          if (
            mutations[0].target.id == "bottom-panel" &&
            mutations[1].target.id == "right-panel"
          ) {
        ...
        if (attributeValue === "active") {
          setTimeout(function () {
            ...
            if (mutationId === "bottom-panel") {
              var panelHeight =
                document.getElementById(mutationId).offsetHeight;
              opendivheight = parseInt(panelHeight, 10);
