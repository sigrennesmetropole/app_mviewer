...
function _disableLayerOpacity(idLayer) {
  ...
  for (var i = 0; i < layersDetails.length; i++) {
    if (layersDetails[i].dataset.layerid === layerId) {
      var mvLayerOptions =
        ".mv-layer-details:eq(" + i + ") > .mv-layer-options";

      $(mvLayerOptions + " > .row").hide();
      if ($(mvLayerOptions + "> .row").length === 1) {
        $(
          ".mv-layer-details:eq(" + i + ") > .layerdisplay-title > a > span",
        ).hide();
      }
    }
  }
...
function _addIconToLayerName(layername, iconContent) {
  for (var i = 0; i < $(".mv-nav-item").length; i++) {
    if ($(".mv-nav-item")[i].dataset.layerid === layername.replace(":", "")) {
      ...
      var newContent = "";
      if (
        iconContent.endsWith(".bmp") ||
        iconContent.endsWith(".dib") ||
        iconContent.endsWith(".jpg") ||
        iconContent.endsWith(".jpeg") ||
        iconContent.endsWith(".jpe") ||
        iconContent.endsWith(".jfif") ||
        iconContent.endsWith(".gif") ||
        iconContent.endsWith(".tif") ||
        iconContent.endsWith(".tiff") ||
        iconContent.endsWith(".png") ||
        iconContent.endsWith(".ico")
      ) {
        newContent =
          '<img src="' +
          iconContent +
        ...
        newContent =
          '<span class="layersIconGlyphicon ' +
          iconContent +
          '" aria-hidden="true"></span><span class="layerText">' +
          text +
          "</span>";
      ...
      var modifiedContent = $(".mv-nav-item > a")[i].innerHTML.replace(
        text,
        newContent,
      );
