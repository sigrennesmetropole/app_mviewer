...
var formatter = (function () {
  ...
  function getElemANotInListB(listA, listB) {
    ...
    for (var i = 0, lenB = listB.length; i < lenB; i++) {
      ...
      for (var j = 0, lenA = elements.length; j < lenA; j++) {
        if (
          listB[i].toUpperCase().trim() === elements[j].toUpperCase().trim()
        ) {
    ...
    for (var cpt = 0, len_cpt = li_tab.length; cpt < len_cpt; cpt++) {
      ...
      if (li_elements.length > 0 || li_ouvert.length > 0) {
        ...
        for (var j = 0, len_j = li_ouvert.length; j < len_j; j++) {
          ...
          const jours = li_ouvert[j].innerHTML.split(" / ");
          for (
            jour_j = 0, len_jours = jours.length;
            jour_j < len_jours;
            jour_j++
          ) {
            if (jours[jour_j].indexOf("Fermé") === -1) {
          ...
          var contenu = "<ul>";
          for (
            lignej = 0, len_lignes = ligne_h.length;
            lignej < len_lignes;
            lignej++
          ) {
        ...
        for (var k = 0, len_k = variations.length; k < len_k; k++) {
          variations[k].style.display = "none";
        }
      } else if (variations.length > 0) {
        //  cas des variations sans grille horaire définie
        variations[0].innerHTML =
  ...
  function fermeturesVacances(li_elements) {
    ...
    if (li_elements.length === l_ref_vacances.length) {
      ...
      if (li_elements.length >= 3) {
        list_retenue.push(
          "Fermé pendant les vacances scolaires sauf " +
            getElemANotInListB(l_ref_vacances, li_elements).join(", "),
        );
      } else if (li_elements.length > 0) {
  ...
  function rmFermeturesOrga() {
    ...
    for (var cpt = 0, len_cpt = li_tab.length; cpt < len_cpt; cpt++) {
      var fermetures_html =
        li_tab[cpt].getElementsByClassName("rm-fermetures")[0];
      ...
      // non affichage des noeuds inutiles
      for (var i = 0, len_i = joursfermes.length; i < len_i; i++) {
        joursfermes[i].style.display = "none";
      }
      for (var k = 0, len_k = comfermeture.length; k < len_k; k++) {
        comfermeture[k].style.display = "none";
      }
    }
  }
        ...
        if (closureInfo) {
          ...
          if (all_closures) {
            for (var i = 0; i < all_closures.length; i++) {
              ...
              var comment =
                elem[2].split(":")[1].replace(/\"/g, "").length > 0
                  ? " : " + elem[2].split(":")[1].replace(/\"/g, "")
                  : "";
              ...
              if (end >= Date.now()) {
                ...
                if (beg < end) {
                  ...
                  output =
                    "<li> Le " + beg.toLocaleDateString() + comment + "</li>";
                }
                closure_array.push(
                  new Array(new Date(beg), new Date(end), output),
                );
              }
            }
  ...
  function rmFermeturesDecheterie() {
    ...
    for (var cpt = 0, len_cpt = li_tab.length; cpt < len_cpt; cpt++) {
      ...
      for (var i = 0, len_ul = ul_element.length; i < len_ul; i++) {
        ...
        var liste_vac = fermeturesVacances(
          ul_element[i].getElementsByClassName("vac_true"),
          ul_element[i].getElementsByClassName("variationvac_true"),
        );
  ...
  function exceptionalClosureDechet() {
    ...
    for (var a = 0; a < elements.length; a++) {
      ...
      if (closureInfo) {
        ...
        for (var i = 0; i < all_closures.length; i++) {
          var closure_dates = all_closures[i].match(
            /[0-9]{2}\/[0-9]{2}\/[0-9]{4}/g,
          );
          var beg = closure_dates[0];
          var end = closure_dates.length > 1 ? closure_dates[1] : beg;
          var comment =
            all_closures[i].split(":").length > 1
              ? " : " + all_closures[i].split(":")[1]
              : "";
          ...
          if (getDateFromFrench(end) >= Date.now()) {
            ...
            if (begFr < endFr) {
              output =
                "- Du " +
                begFr.toLocaleDateString() +
                " au " +
                endFr.toLocaleDateString() +
                comment +
                "</br>";
            } else {
    ...
    if (mypic) {
      for (var i = 0; i < mypic.length; i++) {
        ...
        if (credit !== undefined && credit !== "") {
          ...
          mypic[i].parentNode.getElementsByClassName(
            "text-credit",
          )[0].innerHTML = credit.trimStart();
          mypic[i].parentNode.getElementsByClassName(
            "text-credit",
  ...
  function formatDateInFrench() {
    var span_elements = document.getElementsByClassName(
      "date_in_french_format",
    );
    for (var i = 0; i < span_elements.length; i++) {
      ...
      if (span_elements[i].innerHTML) {
        ...
        if (date_tab.length === 3) {
          span_elements[i].innerHTML =
            date_tab[2].substr(0, 2) + "/" + date_tab[1] + "/" + date_tab[0];
        }
      }
    }
  }
  ...
  function formatLargeNumbers() {
    ...
    for (var i = 0; i < span_elements.length; i++) {
      ...
      if (span_elements[i].innerHTML) {
        contenu = span_elements[i].innerHTML;
        span_elements[i].innerHTML = Number(contenu).toLocaleString(undefined, {
  ...
  function getFeatureFromIdSite(idSite) {
    var features = mviewer
      .getLayers()
      .piscinesRM.layer.getSource()
      .getFeatures();
    var retour;
