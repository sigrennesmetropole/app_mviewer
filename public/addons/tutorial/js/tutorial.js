...
function getExtensionConfigFile() {
  ...
  var extension = configuration
    .getConfiguration()
    .extensions.extension.find((element) => element.id == "tutorial");
  if (extension != undefined) {
    ...
    console.log(
      "Err: L'extension n'a pas pu être trouvée dans la liste des extensions.",
    );
  }
  return configPerso;
}
  ...
  $.getJSON(configFile, function (tutorialData) {
    ...
    for (index in tutorialData) {
      ...
      var tutoHtml =
        ...
        data.order +
        '" class="didacticiel">' +
        '<div class="didact-content" style="width: ' +
        data.width +
        "; top: " +
        data.top +
        "; left: " +
        ...
        '<i class="pin"></i>' +
        '<blockquote class="note postit" style="height: ' +
        data.height +
        '">' +
        data.content +
        "</blockquote>" +
    ...
    const tutoDOMReadyEvent = new CustomEvent("tutoDOM-ready", {
      detail: { nbMessages: ordersTabSorted.length },
    });
...
function _nextmessage() {
  ...
  var activeid = parseInt(
    activeMsg.substr("tutorial".length, activeMsg.length - "tutorial".length),
  );
