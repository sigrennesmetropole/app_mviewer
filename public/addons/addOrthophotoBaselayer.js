...
var _metadata =
  "https://public.sig.rennesmetropole.fr/geonetwork/srv/fre/catalog.search#/home";
...
ortho.layers = _layer;
ortho.attribution =
  "<a href='" + _metadata + "' target='_blank' >Rennes Métropole</a>";

function isBLVisible(conf) {
  if (conf.visible && conf.visible == "true") {
    return true;
  } else {
    return false;
  }
...
function addBaseLayer() {
  ...
  if (l_ext.length) {
    ...
    // une seule extension qui est celle-ci
  ...
  if (configuration.getConfiguration().baselayers.style === "gallery") {
    $("#basemapslist").append(
      Mustache.render(mviewer.templates.backgroundLayerControlGallery, ortho),
    );
    if (!ortho.visible) {
  ...
  if (ortho.visible) {
    // activer la couche si besoin
    mviewer.setBaseLayer(ortho.id);
  } else {
    if (l_bl.length == 2) {
      // ie orthophoto est le fond de plan proposé en second
      showorthothumb();
    }
  }
}
