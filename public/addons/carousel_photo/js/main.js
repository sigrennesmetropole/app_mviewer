var carousel = (function () {
  ...
  function _getpopupslide(idSlider, posSlide, sliderType = "img") {
    ...
    var aRecuperer = document
      .getElementById(idSlider)
      .getElementsByClassName("slick-slide");
    var numPhoto = 1;
    for (var i = 0, len = aRecuperer.length; i < len; i++) {
      if (
        aRecuperer.length == 1 ||
        (aRecuperer[i].getAttribute("role") &&
          aRecuperer[i].getAttribute("role") == "tabpanel")
      ) {
        ...
        if (sliderType == "p") {
          ...
          /*if (blob) {codeHTML +="<a class='img_downld' href='"+ objectURL + "' download>Télécharger l'image</a>";}*/
          codeHTML +=
            "<a class='img_downld' target='_blank' href='" +
            src +
            "' download>Télécharger l'image</a>";
          codeHTML +=
            ...
            "' />";
          codeHTML +=
            "<span class='text-credit car-text-credit'>" +
            credit +
            "</span></center>";
          codeHTML += "</div>";
          numPhoto++;
  ...
  function rmReplaceSliderWithOnePic(slider) {
    ...
    slider.find(".thumbnail").each(function () {
      ...
      $(this).show();
      $(this).append(
        '<p class="labelPictureEnlarge rm-popup-label">Cliquez pour agrandir cette image</p>',
      );
    });
  }
  ...
  function _init() {
    ...
    const targetNode1 = document
      .getElementById("right-panel")
      .getElementsByClassName("popup-content");
    const targetNode2 = document
      ...
      for (const mutation of mutationList) {
        if (mutation.type === "childList") {
          ...
          if (
            !oldTabs ||
            oldTabs.length == 0 ||
            activeTabs.sort().join() != oldTabs.sort().join()
          ) {
