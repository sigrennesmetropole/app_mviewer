const clic = (function() {
  ...
  var _simpleClic = function (evt) {
    console.log('_simpleClic');
  };
  ...
  var addPinOnMap = function (evt) {
    ...
    _currentCoordinates = ol.proj.transform(evt.coordinate, 'EPSG:3857', 'EPSG:4326');
      ...
      _getRightPanel(getIndex(_pinList,_currentCoordinates),_pinList);
  }

  function getIndex(item,coordinates){
    for (let i = 0; i < item.length; i++) {
      if(item[i].values_.source.uidIndex_ != undefined){
        if(item[i].values_.source.uidIndex_[item[i].values_.source.ol_uid-1] != undefined){
          if(item[i].values_.source.uidIndex_[item[i].values_.source.ol_uid-1].values_.coordinates === coordinates){
            return i;
          }
        }
      }
  ...
  function _deletePin(){
    console.log(_map.getLayers().getArray());
    console.log(getIndex(_map.getLayers().array_,_currentCoordinates),);
    _map.getLayers().array_.splice(getIndex(_map.getLayers().array_,_currentCoordinates),1);
    _map.render();
    console.log(_map.getLayers().getArray());
    _closeRightPanel();
  }
  ...
  function _savePin(){
    _currentPin.pinName = $("#pinName")[0].value;
    _currentPin.categorie = $("#pinCategorie")[0].value;
    _currentPin.description = $("#pinDescription")[0].value;
    _currentPin.coordinates[0] = $("#pinLongitude")[0].value;
    _currentPin.coordinates[1] = $("#pinLatitude")[0].value;

    _map.render();
    _getRightPanel(_id,_list);
  }
