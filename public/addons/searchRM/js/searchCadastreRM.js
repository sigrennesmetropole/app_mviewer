  ...
  var init = function () {
    ...
    $.getJSON(baseUrl_cadastre + "communes", function (dataApiJson) {
      ...
      dataApiJson.forEach(function (data) {
        htmlContent +=
          '<option value="' + data.idComm + '">' + data.name + "</option>";
      });
    ...
    $(document).on("change", "#communeSearch", function (e) {
      ...
      // Liste sections
      if (
        typeof codeCom !== "undefined" &&
        codeCom !== null &&
        codeCom.trim() !== ""
      ) {
        $.getJSON(
          baseUrl_cadastre + "communes/" + codeCom + "/sections",
          function (dataApiJson) {
            var htmlContent = "";
            dataApiJson.forEach(function (data) {
              htmlContent +=
                '<option value="' +
    ...
    $(document).on("change", "#section", function (e) {
      ...
      if (
        typeof codeSection !== "undefined" &&
        codeSection !== null &&
        codeSection.trim() !== ""
      ) {
        $.getJSON(
          baseUrl_cadastre + "sections/" + codeSection + "/parcelles",
          function (dataApiJson) {
            var htmlContent = "";
            dataApiJson.forEach(function (data) {
              htmlContent +=
    ...
    $(document).on("change", "#parcelle", function (e) {
      ...
      if (
        typeof codeParcelle !== "undefined" &&
        codeParcelle !== null &&
        codeParcelle.trim() !== ""
      ) {
        $.getJSON(
          baseUrl_cadastre + "epsg:3948/parcelles/" + codeParcelle,
          function (dataApiJson) {
            var geomNewProj = [];
            dataApiJson.coordonnees.geometry.coordinates[0][0].forEach(
              function (coordinates) {
                geomNewProj.push(
                  proj4("EPSG:3948", rmTools.getProjection(), coordinates),
                );
              },
            );
            var geojsonObject = {
            ...
            var e = {
              ...
              pixel: mviewer
                .getMap()
                .getPixelFromCoordinate(mviewer.getMap().getView().getCenter()),
            };
            info.queryMap(e);
          },
        );
