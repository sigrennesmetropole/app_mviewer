mviewer.customLayers.budget_part = (function () {
  ...
  let data_url =
    "https://data.rennesmetropole.fr/api/records/1.0/search?dataset=localisation-et-etat-des-projets-du-budget-participatif&rows=5000&outputFormat=json&refine.projet_remarquable=oui";
      ...
      if (xhr.status === 200 && xhr.responseText) {
        ...
        for (index in response) {
          if (response[index].geometry) {
            ...
            values.geometry = new ol.geom.Point(
              proj4(
                "EPSG:4326",
                "EPSG:3857",
                response[index].geometry.coordinates,
              ),
            );
