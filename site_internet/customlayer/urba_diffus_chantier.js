mviewer.customLayers.urbadiffus_en_chantier = (function () {
  ...
  let data_url =
    "https://public.sig.rennesmetropole.fr/geoserver/wfs?service=WFS&version=1.0.0&request=GetFeature&typeNames=app:tabou_v_oa_programme&outputFormat=application/json&srsName=EPSG:3857";
  ...
  let filter =
    "commune LIKE '%Rennes%' AND nature = 'En diffus' AND diffusion_restreinte=false AND etape='En chantier' AND (nb_logements >= " +
    nb_logements_min +
    " OR nb_logements IS NULL OR nb_logements = 0)";
    ...
    xhr.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        _outsidemarker =
          ...
          encodeURIComponent(doc1.getElementsByTagName("svg")[0].outerHTML);
        _insidemaker =
          "data:image/svg+xml;utf8, " +
          encodeURIComponent(doc2.getElementsByTagName("svg")[0].outerHTML);
      }
    };
  ...
  let dataLayer = new ol.layer.Vector({
    ...
    source: new ol.source.Vector({
      ...
      loader: () => {
        // permet d'éviter le bug de features chargées en double
        fetch(complete_url)
          .then((r) => r.json())
          .then((r) => {
            // nettoie la layer
            mviewer
              .getLayer("urbadiffus_en_chantier")
              .layer.getSource()
              .clear();
            // charge les features
