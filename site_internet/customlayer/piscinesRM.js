mviewer.customLayers.piscinesRM = (function () {
  ...
  let data_site =
    "https://public.sig.rennesmetropole.fr/geoserver/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=v_sitorg_site&outputFormat=application%2Fjson&srsname=EPSG:3857&CQL_FILTER=id_specialite_principale=95%20and%20etat_site=%27actif%27";
  ...
  function getPiscineData(feature) {
    ...
    sitesorgs_data(
      "https://api-sitesorg.sig.rennesmetropole.fr/v1/sites/" +
        feature.get("id_site"),
    ).then(function (result_site) {
      ...
      for (org in piscines_org) {
        ...
        sitesorgs_data(
          "https://api-sitesorg.sig.rennesmetropole.fr/v1/organismes/" +
            piscines_org[org].idOrganisme.idOrganisme,
          piscines_org[org].flagOrganismePrincipal,
        ).then(function (result_org, siteid = site_data.getId()) {
  ...
  function updateFeature(feature, org_data) {
    ...
    org_data.response.horairesOuvertures = cleanedHoraires(
      org_data.response.horairesOuvertures,
    );
    ...
    if (feature != undefined && org_data.org_principal) {
      ...
      feature.set(
        "adresse_postale_adr",
        org_data.response.sites[0].adressePostaleAdr,
      );
      feature.set(
        "adresse_postale_bp_cs",
        org_data.response.sites[0].adressePostaleBpCs,
      );
      feature.set(
        "adresse_postale_cp",
        org_data.response.sites[0].adressePostaleCp,
      );
      feature.set(
        "adresse_postale_comm",
        org_data.response.sites[0].adressePostaleComm,
      );
      feature.set(
        "adresse_postale_cedex",
        org_data.response.sites[0].adressePostaleCedex,
      );
      ...
      org_data.response.nomenclatures[0].idSpecialite.code == "9.2.1"
    ) {
    ...
    if (horaires != undefined) {
      if (horaires) {
        ...
        if (isPiscine) {
          ...
          if (li_fermex != undefined) {
            feature.values_.bassins[bassinNumber].fermetures_excepts =
              li_fermex;
          }
        }
      }
  ...
  function cleanedHoraires(horairesColl) {
    ...
    for (let i = 0; i < horairesColl.length; i++) {
      ...
      if (
        horairesColl[i].dateFin == undefined ||
        horairesColl[i].dateFin == null ||
        new Date(horairesColl[i].dateFin) > now
      ) {
        if (debut > now) {
    ...
    l_cleaned.forEach((item, i) => {
      if (i < l_cleaned.length - 1) {
        if (
          item.dateFin === null &&
          Date.parse(item.dateDebut) < Date.parse(l_cleaned[i + 1].dateDebut)
        ) {
    ...
    xhr.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        stylesrc =
          ...
          encodeURIComponent(doc.getElementsByTagName("svg")[0].outerHTML);
  ...
  let layer = new ol.layer.Vector({
    source: new ol.source.Vector({
      ...
      //url : data_site,
      loader: () => {
        // permet d'éviter les features chargées en double
