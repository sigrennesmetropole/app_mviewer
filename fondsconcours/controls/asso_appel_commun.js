...
const cc = (function () {
  ...
  var data =
    "https://public.sig.rennesmetropole.fr/geoserver/wfs?service=WFS&version=2.0.0&request=GetPropertyValue&outputFormat=application/json&typeNames=eq_autres:asso_appel_commun&valueReference=edition";
  var l_editions = [];
      ...
      xhr.onload = function () {
        if (xhr.status === 200 && xhr.responseText) {
          ...
          var response = xhr.responseText.length
            ? parser.parseFromString(xhr.responseText, "application/xml")
            : null;
  ...
  return {
    ...
    init: function () {
      ...
      getFeaturesData().then(function () {
        ...
        var customselector = document.getElementById(
          "asso_appel_commun-attributes-selector",
        );
