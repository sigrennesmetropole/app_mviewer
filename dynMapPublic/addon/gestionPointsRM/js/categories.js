...
const categories = (function () {
  ...
  function _setSelectedShapeURL(categorieid, url) {
    $("#" + categorieid + "-markershape option[value='" + url + "']").prop(
      "selected",
      true,
    );
  }
  ...
  function _updateCategorie(id) {
    ...
    _calculateStyle(
      ...
      formeSrc,
      couleur,
      _getCategorieById(id).getIconeAnchor(),
    );
  ...
  function _refreshSvgMarker(categorieId) {
    _showSvgMarker(
      "showmarker" + categorieId,
      _getSelectedShapeURL(categorieId),
      _getSelectedColor(categorieId),
    );
  }
      ...
      response
        ...
        .then((xml) => {
          ...
          if (svgWidth > maxwidth) {
            return maxwidth / svgWidth;
          } else {
            return 1;
          }
        })
        .then((echelle) => {
          localisations.updateStyle(
            id,
            _getOLStyle(couleur, url, echelle, anchor),
          );
        })
        .then(() => {
          _refreshLegend();
  ...
  function _showAllStyles() {
    ...
    for (categ in allcategories) {
      _addCategInConfigWindow(
        allcategories[categ].getId(),
        allcategories[categ].getNom(),
        allcategories[categ].getIconeURL(),
        allcategories[categ].getCouleur(),
      );
    }
  }
  ...
  function _addCategInConfigWindow(id, nom, iconeURL, couleur) {
    ...
    var html =
      "<tr categorieId=" +
      id +
      '><td><input type="text" class="categNameInput" id="' +
      id +
      '-nameconf" onchange="categories.updateCategorie(' +
      id +
      ');"  maxlength="60" /> </td>';
    ...
    html +=
      '    <select id="' +
      id +
      '-markershape" class="icones-value" onchange="categories.updateCategorie(' +
      id +
      ');">';
    ...
    html +=
      '    <input id="' +
      id +
      '-markercolor" type="color" list="presetColors" onchange="categories.updateCategorie(' +
      id +
      ');" />';
    html +=
      '      <datalist id="presetColors"><option>#f78b12</option><option>#e04a3c</option><option>#33919d</option><option>#aa0c8b</option><option>#737543</option><option>#3e476c</option></datalist>';
    html +=
      '<svg id="showmarker' +
      id +
      '" preserveAspectRatio="xMidYMid meet" class="confImg" ></svg></td>';
    html +=
      '<td class="alig-center"><button id="delcategbtn' +
      id +
      '" type="button" class="btn btn-default navbar-btn mv-navbar-btn" data-bs-toggle="modal" data-bs-target="#delete-modal" i18n="help.modal.doc" title="Supprimer la catégorie"><i class="btn fa fa-trash-alt" ></i></button></td></tr>';
    ...
    if (newCateg == true) {
      allcategories.push(
        new Categorie(
          id,
          $("#" + id + "-nameconf").attr("value"),
          $("#" + id + "-markershape option:selected").text,
          couleur,
        ),
      );
      _updateCategorie(id);
  ...
  function _initDefaultCategConfig() {
    ...
    html +=
      '    <label id="default-markershape" class="icones-value" >' +
      iconName +
      "</label>";
    html +=
      '    <input id="default-color" type="color" value="' +
      couleurHex +
      '" disabled />';
    html +=
      '<svg id="default-showmarker" preserveAspectRatio="xMidYMid meet" class="confImg" ></svg></td>';
    html += "<td></td></tr>";
  ...
  function _updateFicheInfoCateg() {
    ...
    for (index in allcategories) {
      ...
      $(".categ-choice").append(
        new Option(
          allcategories[index].getNomForList(),
          allcategories[index].getId(),
          selected,
          selected,
        ),
      );
    ...
    $(".categ-choice").prepend(
      new Option(
        "-- Sans catégorie (défaut) --",
        "0",
        !categorieTrouvee,
        !categorieTrouvee,
      ),
  ...
  function _refreshLegend() {
    ...
    for (categ in allcategories) {
      ...
      html +=
        '  <td><svg id="legendmarker' +
        id +
        '" preserveAspectRatio="xMidYMid meet" class="confImg" ></svg></td>';
      html +=
        '  <td><span class="legendcategname">' +
        allcategories[categ].getNomForList() +
        "</span></td>";
      html += "</tr>";
    ...
    let htmldef = "<tr>";
    htmldef +=
      '  <td><svg id="legendmarker-default" preserveAspectRatio="xMidYMid meet" class="confImg" ></svg></td>';
    htmldef +=
      '  <td><span class="legendcategname"> Points sans catégorie </span></td>';
    htmldef += "</tr>";
  ...
  function _initGUI() {
    $(document).ready(function () {
      ...
      $(
        "#legend li[data-layerid='meslocalisations'] div.layerdisplay-legend",
      ).append($("#legend-localisations"));

      // Bouton de creation de point
      _createBtn = new ol.Overlay({
        positioning: "top-center",
        element: $("#createPointOnMapTooltip")[0],
        stopEvent: true,
      });
...
class Categorie {
  ...
  getNomForList() {
    return this.nom == undefined || this.nom.trim() == ""
      ? "Catégorie sans nom #" + this.id
      : this.nom;
  }
  setNom(nom) {
    this.nom = nom;
