  ...
  reader.addEventListener(
    ...
    () => {
      ...
      if (file.name.split(".").at(-1) == "geojson") {
        try {
          [objetConvertPoints, objetConvertLignes] = conversionJSON(
            reader.result,
          );
          ...
          const elements = document.querySelectorAll(
            ".boutonEnvoyer, .Panneauconfiguration, .messageCarte, #map",
          );
          ...
          const SelectAttributIdBalade =
            document.querySelector("#attributIdBalade");
          while (SelectAttributIdBalade.firstChild) {
            SelectAttributIdBalade.removeChild(
              SelectAttributIdBalade.firstChild,
            );
          }
          ...
          Object.keys(objetConvertLignes.features[0].properties).forEach(
            (attribut) => {
              const option = document.createElement("option");
              option.value = attribut;
              option.innerText = attribut;
              if (/id/i.test(attribut)) option.selected = true;
              SelectAttributIdBalade.appendChild(option);
            },
          );

          // Attribut ID de chaque point
          const SelectAttributIdPoint =
            document.querySelector("#attributIdPoint");
          ...
          Object.keys(objetConvertPoints.features[0].properties).forEach(
            (attribut) => {
              const option = document.createElement("option");
              option.value = attribut;
              option.innerText = attribut;
              if (/balade/i.test(attribut)) option.selected = true;
          ...
          document
            .querySelector("#baladeDefautSelectionnes")
            .classList.add("hidden");

          // Attribut Rang de chaque point
          const SelectAttributRangPoint =
            document.querySelector("#attributRang");
          while (SelectAttributRangPoint.firstChild) {
            SelectAttributRangPoint.removeChild(
              SelectAttributRangPoint.firstChild,
            );
          }
          var optionSelectionnerListe = document.createElement("option");
          optionSelectionnerListe.value = "";
          ...
          Object.keys(objetConvertPoints.features[0].properties).forEach(
            (attribut) => {
              ...
              option.value = attribut;
              option.innerText = attribut;
              if (/rang|ordre/i.test(attribut)) option.selected = true;
              SelectAttributRangPoint.appendChild(option);
            },
          );

          // Attribut CouleurBalade de chaque tracés
          const SelectAttributCouleurBalade = document.querySelector(
            "#attributCouleurBalade",
          );
          while (SelectAttributCouleurBalade.firstChild) {
            SelectAttributCouleurBalade.removeChild(
              SelectAttributCouleurBalade.firstChild,
            );
          }
          var optionSelectionnerListe = document.createElement("option");
          optionSelectionnerListe.value = "";
          ...
          Object.keys(objetConvertLignes.features[0].properties).forEach(
            (attribut) => {
              ...
              option.value = attribut;
              option.innerText = attribut;
              if (/^couleur$|^color$/i.test(attribut)) option.selected = true;
              SelectAttributCouleurBalade.appendChild(option);
            },
          );

          // Attribut CouleurPoint de chaque points
          const SelectAttributCouleurPoint = document.querySelector(
            "#attributCouleurPoint",
          );
          while (SelectAttributCouleurPoint.firstChild) {
            SelectAttributCouleurPoint.removeChild(
              SelectAttributCouleurPoint.firstChild,
            );
          }
          var optionSelectionnerListe = document.createElement("option");
          optionSelectionnerListe.value = "";
          ...
          const SelectAttributBaladeDefaut = document.querySelector(
            "#baladeDefautSelectionnes",
          );
          while (SelectAttributBaladeDefaut.firstChild) {
            SelectAttributBaladeDefaut.removeChild(
              SelectAttributBaladeDefaut.firstChild,
            );
          }
          ...
          document.getElementById("message").innerHTML =
            "Les données du fichier ne sont pas valides.";
          const elements = document.querySelectorAll(
            ".boutonEnvoyer, .Panneauconfiguration, .messageCarte, #map",
          );
          for (let element of elements) {
        ...
        document.getElementById("message").style.display = "block";
        document.getElementById("message").innerHTML =
          "Le fichier n'est pas sous le bon format GeoJSON.";
        const elements = document.querySelectorAll(
          ".boutonEnvoyer, .Panneauconfiguration, .messageCarte, #map",
        );
        for (let element of elements) {
...
function conversionJSON(objetTexte) {
  ...
  objetJSON.features.forEach((feature) => {
    if (feature.geometry.type == "Point") {
      feature.geometry.coordinates = convertCoordinates4326to3857(
      ...
      feature.geometry.coordinates = feature.geometry.coordinates.map(
        (coordinate) =>
          convertCoordinates4326to3857(coordinate[0], coordinate[1]),
      );
      for (const [key, value] of Object.entries(feature.properties)) {
        if (/^\d+$/.test(value)) {
          feature.properties[key] = parseInt(value);
  ...
  layers: [
    new ol.layer.Tile({
      source: new ol.source.WMTS({
        ...
        format: "image/png",
        attributions:
          "&lt;a href=&quot;https://public.sig.rennesmetropole.fr/geonetwork/srv/fre/catalog.search#/metadata/2ff4b02a-7d1e-4e9c-a0c2-dddbb11a3168&quot; target=&quot;_blank&quot; &gt;Rennes Métropole&lt;/a&gt;",
...
function setColorOnMap() {
  ...
  try {
    vectorLayerPoints.getSource().forEachFeature((feature) => {
      ...
      var idBalade =
        feature.get("values")[document.querySelector("#attributIdPoint").value];
      ...
      if (document.querySelector("#couleurPointFixe-couleur").checked)
        ...
        var couleurBalade = map
          .getLayers()
          .getArray()[2]
          .getSource()
          .getFeatures()
          .find((feature) => feature.get("values")[idNameBalade] == idBalade)
          .get("values")[document.querySelector("#attributCouleurPoint").value];
...
document
  .querySelectorAll("#attributIdPoint, #attributIdBalade")
  .forEach((element) => {
...
function setBaladeParDefaut() {
  ...
  map
    ...
    .forEachFeature((feature) => {
      var idBalade =
        feature.get("values")[
          document.querySelector("#attributIdBalade").value
        ];
      var titre = feature.get("values")["name"];
      const option = document.createElement("option");
      option.value = idBalade;
      option.innerText =
        titre != null ? "id : " + idBalade + " / " + titre : "id : " + idBalade;
      SelectAttributBaladeDefaut.appendChild(option);
    });
}
document
  .querySelector("#attributIdBalade")
  .addEventListener("change", setBaladeParDefaut);
document
  .querySelector("#ouvertureBalade-oui")
  .addEventListener("change", setBaladeParDefaut);

// Gestion de l'affichage des points non séléctionnés
document
  .querySelector("#affichagePointNonSelect")
...
document
  .querySelector("#hexaCouleurPointActif")
  .addEventListener("input", () => {
    if (
      document
        .getElementById("hexaCouleurPointActif")
        .value.match(/^#[a-f0-9]{6}$/i) !== null
    )
      ...
      feature.setStyle(
        new ol.style.Style({
          stroke: new ol.style.Stroke({
            ...
            color: isColor(
              feature.get("values")[
                document.querySelector("#attributCouleurBalade").value
              ],
            )
              ? feature.get("values")[
                  document.querySelector("#attributCouleurBalade").value
...
document
  .querySelector("#hexaCouleurBaladeFixe")
  .addEventListener("input", () => {
    if (
      document
        .getElementById("hexaCouleurBaladeFixe")
        .value.match(/^#[a-f0-9]{6}$/i) !== null
    )
...
document
  .querySelector("#attributCouleurBalade")
  .addEventListener("change", couleurBaladeFixe);
...
document
  .querySelector("#hexaCouleurPointFixe")
  .addEventListener("input", () => {
    if (
      document
        .getElementById("hexaCouleurPointFixe")
        .value.match(/^#[a-f0-9]{6}$/i) !== null
    )
...
document
  .querySelector("#attributCouleurPoint")
  .addEventListener("change", couleurPointFixe);
...
document.querySelector("#zoomBalade").addEventListener("change", (e) => {
  ...
  document.querySelector("#boutonRetourZoom").addEventListener("click", () => {
    ...
    map
      .getView()
      .animate({ zoom: lastZoom, center: lastCenter, duration: 800 }, () => {
        ...
        document.querySelector("#boutonEnvoyer").style.backgroundColor =
          "#DC2626";
      });
  });
...
document.getElementById("couleurPointActif").addEventListener("input", () => {
  document.getElementById("hexaCouleurPointActif").value =
    document.getElementById("couleurPointActif").value;
});
document
  .getElementById("hexaCouleurPointActif")
  .addEventListener("input", () => {
...
document.getElementById("couleurBaladeFixe").addEventListener("input", () => {
  document.getElementById("hexaCouleurBaladeFixe").value =
    document.getElementById("couleurBaladeFixe").value;
});
document
  .getElementById("hexaCouleurBaladeFixe")
  .addEventListener("input", () => {
...
document.getElementById("couleurPointFixe").addEventListener("input", () => {
  document.getElementById("hexaCouleurPointFixe").value =
    document.getElementById("couleurPointFixe").value;
});
document
  .getElementById("hexaCouleurPointFixe")
  .addEventListener("input", () => {
...
document
  .querySelector("#couleurBaladeFixe-couleur")
  .addEventListener("click", () => {
    document.querySelector("#couleurBaladeFixe").classList.remove("hidden");
    document.querySelector("#labelCouleurBaladeFixe").innerHTML =
      "Couleur des tracés :";
...
document
  .querySelector("#couleurBaladeFixe-attribut")
  .addEventListener("click", () => {
...
document
  .querySelector("#couleurPointFixe-couleur")
  .addEventListener("click", () => {
    document.querySelector("#couleurPointFixe").classList.remove("hidden");
    document.querySelector("#labelCouleurPointFixe").innerHTML =
      "Couleur des points :";
...
document
  .querySelector("#couleurPointFixe-attribut")
  .addEventListener("click", () => {
...
document
  .querySelector("#envoyerFormulaireConfirm")
  .addEventListener("click", () => {
    ...
    if (formProjet.checkValidity()) {
      ...
      if (document.querySelector("#ouvertureBalade-oui").checked)
        baladeParDefaut = document.querySelector(
          "#baladeDefautSelectionnes",
        ).value;
      if (document.querySelector("#couleurBaladeFixe-couleur").checked)
        couleurBaladeFixe = document.querySelector("#couleurBaladeFixe").value;
      else
        attributCouleurBalades = document.querySelector(
          "#attributCouleurBalade",
        ).value;
      if (document.querySelector("#couleurPointFixe-couleur").checked)
        couleurPointFixe = document.querySelector("#couleurPointFixe").value;
      else
        attributCouleurPoints = document.querySelector(
          "#attributCouleurPoint",
        ).value;

      fichiers["param_" + uid + ".json"] = {
        carte: {
          zoomPendantBalade: parseInt(form.elements["zoomBalade"].value),
        },
        balades: {
          id: form.elements["attributIdBalade"].value,
          couleurBalades: attributCouleurBalades,
          couleurPoints: attributCouleurPoints,
          couleurBaladeFixe: couleurBaladeFixe,
          couleurPointFixe: couleurPointFixe,
          defaultColor: defaultColor,
          baladeParDefaut: baladeParDefaut,
        },
        points: {
          idBalade: form.elements["attributIdPoint"].value,
          champRang: form.elements["attributRang"].value,
          couleurPointActif: form.elements["couleurPointActif"].value,
          pointsVisible:
            form.elements["affichagePointNonSelect"].value == "Oui"
              ? "true"
              : "false",
        },
      };
      ...
      fetch("envoiMail.php", {
        ...
        window.location.href =
          window.location.href.split("?")[0] + "?mail=" + mail;
      });
    } else {
...
window.onload = function () {
  ...
  if (mail) {
    document.querySelector("#messageConfirmationText").innerHTML =
      "Votre demande (<span class='font-medium text-gray-900'>" +
      mail +
      "</span>) a été envoyé au service SIG.";
    document.querySelector("#messageConfirmation").classList.remove("hidden");
    document
      .querySelector("#buttonCloseMessageConfirmation")
      .addEventListener("click", () => {
