var baladesAddon = (function () {
  ...
  var init = function () {
    ...
    if (getParametreVisible()) geolocalisation();
  };
  ...
  function geolocalisation() {
    ...
    if (getParametreVisible()) {
      if (_map.getSize()[0] > 1000) {
        ...
        document
          .getElementById("geolocbtnBalade")
          .addEventListener("click", onclickGeolocalisation);
      }
    }
  }
  ...
  function createLayerHighlight() {
    ...
    HighlightLayer.mviewerid = "featOverlay";
  ...
  var clickPrevButtonFunction = function clickPrevButton() {
    currentPointBalade--;
    var geometryPointFeature = featuresPoints
      .find(
        (x) =>
          x.get(idBalade) == currentIdBalade &&
          x.get(champRang) == currentPointBalade,
      )
      .getGeometry()
      .getCoordinates();
    if (geometryPointFeature) {
      ...
      mviewer.zoomToLocation(
        geometryPointFeature[0],
        geometryPointFeature[1] + 0.0002,
        zoomPendantBalade,
        true,
        "EPSG:3857",
      );
      ...
      highlightLayer
        ...
        .getFeatures()[0]
        .getGeometry()
        .setCoordinates([geometryPointFeature[0], geometryPointFeature[1]]);

      opendivheight =
        document.getElementById("modal-panel").children[0].offsetHeight;
      setTimeout(() => {
    ...
    if (
      !featuresPoints.find(
        (x) =>
          x.get(idBalade) == currentIdBalade &&
          x.get(champRang) == currentPointBalade - 1,
      )
    ) {
  ...
  var setInfoPanelTitleFunction = function setInfoPanelTitle(el, panel) {
    ...
    var idBaladeSelected = $(
      "#" + panel + " " + $(el).attr("href") + " #idbalade",
    ).text();
    currentIdBalade = idBaladeSelected;
    featuresBalades.map((feat) => {
      if (feat.get("id") != idBaladeSelected) {
        var colorFeature =
          feat.getStyle().getStroke().getColor().slice(0, 7) + "66";
      } else {
  ...
  function changeOpacityOnClick(e) {
    ...
    var feature = map.forEachFeatureAtPixel(e.pixel, function (feature, layer) {
      if (!layer.mviewerid || layer.mviewerid != highlightLayer.mviewerid) {
        return feature;
      }
    });
    ...
    featuresBalades.map((feat) => {
      if (
        (feature &&
          ...
          feat.get("id") != feature.get("id")) ||
        (feature &&
          feature.getGeometry().getType() == "Point" &&
          feat.get(idBalade) != feature.get(idBalade))
      ) {
        var colorFeature =
          feat.getStyle().getStroke().getColor().slice(0, 7) + "66";
      } else {
    ...
    featuresPoints.map((feat) => {
      if (
        ...
        if (feature || pointsVisible == "true")
          feat.getStyle().getImage().setOpacity(1);
        else feat.getStyle().getImage().setOpacity(0);
      }
    });
    mviewer.customLayers[layer_points].layer.changed();
    // if (!feature)
    ...
    if (feature && feature.getGeometry().getType() == "Point") {
      if (feature.get(idBalade)) {
        ...
        var geometryPointFeature = featuresPoints
          ...
          .getGeometry()
          .getCoordinates();
        geometryPoint = ol.proj.transform(
          [geometryPointFeature[0], geometryPointFeature[1]],
          "EPSG:3857",
          "EPSG:4326",
        );
        mviewer.getMap().removeLayer(highlightLayer);
        ...
        setTimeout(() => {
          _map
            .getView()
            .setCenter([geometryPointFeature[0], geometryPointFeature[1]]);
          // decaleMap([0, opendivheight/3]);
        }, 400);
      } else {
        currentIdBalade = -1;
  ...
  var styleBalades = function () {
    featuresBalades = mviewer.customLayers[layer_balades].layer
      .getSource()
      .getFeatures();
    featuresPoints = mviewer.customLayers[layer_points].layer
      .getSource()
      .getFeatures();
    ...
    var features = mviewer.customLayers[layer_balades].layer
      .getSource()
      .getFeatures();
    var featuresCouleur = [];
    ...
    features.forEach((balade) => {
      ...
      if (!isColor(couleurFeature)) couleurFeature = defaultColor;
      if (isColor(couleurBaladeFixe)) couleurFeature = couleurBaladeFixe;
      var style = new ol.style.Style({
    ...
    features = mviewer.customLayers[layer_points].layer
      .getSource()
      .getFeatures();
    features.forEach((point) => {
      var couleurPoint = featuresCouleur
        .find((x) => x["id"] == point.get(idBalade))
        .balade.get(couleurPoints);
      if (!isColor(couleurPoint)) couleurPoint = defaultColor;
      if (isColor(couleurPointFixe)) couleurPoint = couleurPointFixe;
      var style = new ol.style.Style({
        image: new ol.style.Icon({
          color: couleurPoint,
          crossOrigin: "anonymous",
      ...
      if (pointsVisible == "false") point.getStyle().getImage().setOpacity(0);
    });

    // Gestion de l'option de la balade par défaut (defaut)
    if (baladeParDefaut != "") {
      var featureDefaut = mviewer.customLayers[layer_points].layer
        .getSource()
        .getFeatures()
        .find(
          (x) => x.get(idBalade) == baladeParDefaut && x.get(champRang) == 1,
        );
      if (featureDefaut) {
        var geometryPoint = ol.proj.transform(
          [
            featureDefaut.getGeometry().getCoordinates()[0],
            featureDefaut.getGeometry().getCoordinates()[1],
          ],
          "EPSG:3857",
          "EPSG:4326",
        );
        mviewer.zoomToLocation(
          geometryPoint[0] + 0.00002,
          geometryPoint[1] + 0.00002,
          zoomPendantBalade,
          true,
        );
        highlightLayer
        ...
        console.log(
          "Extension balades : id de la balade par défaut non valide",
        );
      }
    }
  };
